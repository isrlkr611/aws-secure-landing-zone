...
2. **Brique 2**: compliance mapping - reuse findings from Brique 1 (and a future cloud posture scanner) against a `mapping-normes.yaml` linking each finding type to ISO 27001 / PCI-DSS / NIS2 / SOC 2 clauses, producing a per-standard compliance percentage.
3. **Brique 3**: purple-team simulation - Atomic Red Team techniques run against the operator's own lab only (never a customer's infrastructure without a separate contract), verifying whether the detection stack (GuardDuty/Sentinel, Wazuh/Suricata) actually alerts.
